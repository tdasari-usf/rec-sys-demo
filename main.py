...
class Recommender:
  ...
  def process_articles(self):
    ...
    unique_articles = articles.drop_duplicates(subset = "title", keep = 'first')
    ...
    self.idx_to_id = {idx : content_id for idx, content_id in enumerate(indexes)}
    self.id_to_idx = {content_id : idx for idx, content_id in enumerate(indexes)}
  ...
  def process_interactions(self):
    ...
    interactions_english = interactions[interactions['contentId'].isin(filter_en)]
  ...
  def create_similar_items(self):
    ...
    for idx_1, _ in enumerate(sentences):
      ...
      for idx_2, _ in enumerate(sentences):
        if idx_1 == idx_2:
          continue
        sim_dict[idx_1][idx_2] = cos_sim(vect_dict[idx_1], vect_dict[idx_2])
      
      sorted_items = sorted(sim_dict[idx_1].items(), key = lambda x:x[1], reverse = True)
  ...
  def create_popular_items(self):
    ...
    self.interactions['score'] = self.interactions['eventType'].apply(lambda x: event_type_strength[x])
    scored_content = self.interactions.groupby('contentId').count()
    popular_content = scored_content.sort_values('score', ascending = False).index.tolist()
